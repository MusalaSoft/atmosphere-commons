...
public class WebSocketCommunicatorManager {
    ...
    private static Map<String, ClientServerResponse> responseMap;

    private WebSocketCommunicatorManager() {
        lockObjectMap = new ConcurrentHashMap<String, Object>();
        responseMap = new ConcurrentHashMap<String, ClientServerResponse>();
    }
    ...
    public ClientServerResponse getServerResponse(String sessionId) {
        if (!responseMap.containsKey(sessionId)) {
            return null;
        }
        ClientServerResponse response = responseMap.get(sessionId);
        responseMap.remove(sessionId);
        return response;
    }

    public void setServerResponse(ClientServerResponse response) {
